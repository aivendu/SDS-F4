...
SD_Error SD_Init(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PREEMPTION_PRIORITY_0; //抢占优先级3
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = SUB_PRIORITY_SDCARD;      //子优先级3
...
**            buf_len 无效参数
** 输 出 ：
** 全局变量:Data595 -- 缓存写到595端口上的数据
** 调用模块:
**
** 作　者: 杜其俊
** 日　期: 2014年8月13日
********************************************************************************/
int8_t SDCardOpen(int32_t port, void *config, uint8_t len)
{
    SD_Init();
    return 0;
}
...
** 日　  期 :   2017/07/03
*****************************************************************************************/
int8_t SDCardClose(int32_t port)
{
    return 0;
}
...
** 日　  期 :   2017/07/03
*****************************************************************************************/
int8_t SDCardWrite(int32_t port, void *buf, uint16_t buf_len)
{
    return 0;
}
...
** 日　  期 :   2017/07/03
*****************************************************************************************/
int16_t SDCardRead(int32_t port, void *buf, uint16_t buf_len)
{
    //    uint8_t res=0;
...
** 日　  期 :   2017/07/03
*****************************************************************************************/
int8_t SDCardIoctl(int32_t port, uint32_t cmd, va_list argp)
{
    ...
    case SD_CMD_WRITEDISK:
    {
        uint8_t *buf;
        uint32_t sector;
        uint8_t cnt;
        buf = va_arg(argp, void *);
        sector = va_arg(argp, int);
        cnt = va_arg(argp, int);
        return_flag = SD_WriteDisk(buf, sector, cnt);
    }
    break;

    case SD_CMD_READDISK:
    {
        uint8_t *buf;
        uint32_t sector;
        uint8_t cnt;
        ...
        sector = va_arg(argp, int);
        cnt = va_arg(argp, int);
        return_flag = SD_ReadDisk(buf, sector, cnt);
    }
    break;
