...
{
    SD_CMD_INIT,		                //  初始化
    SD_CMD_WRITEDISK,						//	写扇区
    SD_CMD_READDISK,				//	读扇区

}e_SDCardCmdCode_e;


extern int8_t SDCardOpen(int32_t port, void *config, uint8_t len);
extern int8_t SDCardClose(int32_t port);
extern int8_t SDCardWrite(int32_t port, void *buf, uint16_t buf_len);
extern int16_t SDCardRead(int32_t port, void *buf, uint16_t buf_len);
extern int8_t SDCardIoctl(int32_t port, uint32_t cmd, va_list argp);


#define INTX_DISABLE()   OS_ENTER_CRITICAL() //关闭总中断(POLLING模式,严禁中断打断SDIO读写操作!!!)
#define INTX_ENABLE()    OS_EXIT_CRITICAL()  //开启总中断
