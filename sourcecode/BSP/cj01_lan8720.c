...
uint8_t LAN8720_Init(void)
{
    ...
    GPIO_ResetBits(LAN8720_RST_GPIO, LAN8720_RST_PIN);//硬件复位LAN8720
	OSTimeDly(OS_TICKS_PER_SEC/20);	
    GPIO_SetBits(LAN8720_RST_GPIO, LAN8720_RST_PIN);//复位结束 

	
	NVIC_InitStructure.NVIC_IRQChannel = ETH_IRQn;  //以太网中断
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = PREEMPTION_PRIORITY_0;  //中断寄存器组2最高优先级
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = SUB_PRIORITY_ETH;
