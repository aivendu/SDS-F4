...
** 函 数 名 称 ：Crc16Update
** 函 数 功 能 ：CRC校验
** 输 入 ：crc_data：要计算的CRC数据
**       ：crc_data_len：CRC数据的长度
** 输 出 ：0：初始化成功
**       ：-1：初始化失败
** 全局变量：
** 调用模块：
...
** 日　期: 2014年4月08日
*****************************************************************************************/
uint16_t CRC16(uint16_t crc, uint8_t *crc_data, uint8_t crc_data_len)
{
    uint8_t tmp;      // CRC循环中的索引

    while (crc_data_len--)
    {
        // 计算CRC
        tmp = (crc) ^ *crc_data++;
        crc = (crc>>8) ^ crc_tab16[tmp];
    }
...
uint32_t crc_32(uint8_t *data, uint32_t len)
{
	...
	while(uIndex--)
	{
		...
		uData = __REV((u32*)data);
#else
		memcpy((uint8_t*)&uData,data,4);
#endif		
		data += 4;
		uData = __RBIT(uData);
		CRC->DR = uData;
...
#endif
static uint16_t CCITT_CRCByte(uint16_t crc_date, uint8_t b)
{
    crc_date = (uint8_t)(crc_date >> 8) | (crc_date << 8);
    crc_date ^= b;
    crc_date ^= (uint8_t)(crc_date & 0xff) >> 4;
    crc_date ^= crc_date << 12;
    crc_date ^= (crc_date & 0xff) << 5;
    return crc_date;
}
...
{
    uint8_t i = 0;
    uint16_t crc_data = crcx;
    for(i = 0; i < n; i++)
    {
        crc_data = CCITT_CRCByte(crc_data, *b++);
    }
    return crc_data;
}
